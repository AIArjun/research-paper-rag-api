...
| Token bound | The reservation is an explicit model-supported upper bound, never a character heuristic: OpenAI models use the exact tiktoken encoding for the configured model (o200k_base for gpt-4o-mini) plus 16 framing tokens for the chat envelope plus the enforced output cap; Ollama uses the UTF-8 byte length (byte-level tokenizers never emit more tokens than bytes) plus the same framing and cap. A model tiktoken cannot map, an encoding that cannot be loaded, or a missing tiktoken package fails closed before any backend loads (`token_bound_unavailable` / `missing_dependency`). `/ready` and `model_usage.reservation_bound` name the bound in use | `app/tokens.py`, `Dockerfile.real` (baked encoding cache) |
...
- Token reservations are computed from a model-supported bound: for OpenAI, the exact tiktoken count of the prompt plus 16 framing tokens plus the output cap (verified against tiktoken 0.14.0: 2000 CJK characters or 2000 emoji tokenize to about 2060 input tokens, where a three-characters-per-token heuristic reserved about 762 for the whole prompt); for Ollama, the UTF-8 byte length plus the same framing and cap. Provider-reported usage replaces the reservation after the call where the client exposes `usage_metadata` (OpenAI chat models); a measured total larger than the reservation is charged in full and logged as an overshoot so later calls see it. Ollama string responses stay at the reservation.
